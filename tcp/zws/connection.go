		...
		case data, ok := <-c.msgBuffChan:
			if ok {
				...
				if data.ID != "pb_battle.MsgNtfBattleFrames" {
					logx.DebugF("SendBuffMsg success, ConnID = %d, msgName = %s", c.ConnID, data.ID)
				}
			} else {
...
func (c *Connection) sendRest() {
	defer logx.Debugln(c.RemoteAddr().String(), "[sendRest!]")
	for {
		select {
		case data, ok := <-c.msgBuffChan:
			if !ok || data == nil {
...
func (c *Connection) Start() {
	...
	logx.Debugln(c.RemoteAddr().String(), "[running!]")
	<-c.ctx.Done()
	logx.Debugln(c.RemoteAddr().String(), "[stoping!]")
...
func (c *Connection) SendBuffMsg(msgID string, data []byte) error {
	...
	//写回客户端
	c.msgBuffChan <- zcommon.NewMsgPackage(msgID, data)
