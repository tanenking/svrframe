...
func (mh *MsgHandle) SendMsgToTaskQueue(request ziface.IRequest) {
	...
	//logx.Debugln("Add ConnID=", request.GetConnection().GetConnID()," request msgID=", request.GetMsgID(), "to workerID=", workerID)
	//将请求消息发送给任务队列
...
func (mh *MsgHandle) DoMsgHandler(request ziface.IRequest) {
	...
	if handler == nil {
		logx.Debugln("api msgID = ", request.GetMsgID(), " is not FOUND!")
		return
	}
...
func (mh *MsgHandle) RegisterRouter(msgID string, router ziface.IRouter) {
	...
	logx.Debugln("Add api msgID = ", msgID)
}

// 路由功能: 没有指定的消息,都通过这个处理
func (mh *MsgHandle) RegisterGlobalRouter(router ziface.IRouter) {
	mh.GlobalApi = router
}

// StartOneWorker 启动一个Worker工作流程
func (mh *MsgHandle) StartOneWorker(workerID int, taskQueue chan ziface.IRequest) {
	logx.Debugln("Worker ID = ", workerID, " is started.")
	//不断的等待队列中的消息
